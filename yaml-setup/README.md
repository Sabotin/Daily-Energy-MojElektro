...
This is the manual way: the same dashboard built from a card file, two API requests, a script and two automations.
...
| [`automation-logger.yaml`](automation-logger.yaml) | Automation (backup): logs the same values from the Moj Elektro sensors |
| [`daily-energy-card.js`](daily-energy-card.js) | The dashboard card |
| [`dashboard-view.yaml`](dashboard-view.yaml) | The dashboard view |

## Before you start

- The [Moj Elektro integration](https://github.com/frlequ/homeassistant-mojelektro) by frlequ, set up with your account
  (the logger automation reads its sensors).
- Your Moj Elektro **API token** and **meter ID** (for example `4-123456`, the "Merilno mesto" number in the portal).
...
5. **Automations** – Settings → Automations & scenes → Create automation → ⋮ → Edit in YAML, once for
   [`automation-api.yaml`](automation-api.yaml) and once for [`automation-logger.yaml`](automation-logger.yaml).
...
- The **logger** automation is a backup: it saves the values from the Moj Elektro sensors when they change.
