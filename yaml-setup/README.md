...
This is the manual way: the same dashboard built from a card file, two API requests, a script and an automation.
...
5. **Automation** – Settings → Automations & scenes → Create automation → ⋮ → Edit in YAML, paste
   [`automation-api.yaml`](automation-api.yaml), save.
...
- The manual meter reading and "Delete all data" (Settings) ask for a PIN; change `const DEL_PIN = '4085';` near the top of
  the card file.
